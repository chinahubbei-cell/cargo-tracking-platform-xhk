...
func (o *Order) BeforeCreate(tx *gorm.DB) error {
	...
	if o.OrderNo == "" {
		o.OrderNo = generateOrderNoV2(o.OrderType)
	}
	return nil
}

func generateOrderNoV2(orderType string) string {
	now := time.Now()
	prefix := "PO" // purchase order
	if orderType == "renewal" {
		prefix = "RN" // renewal
	}
	return prefix + "-" + now.Format("060102") + "-" + uuid.New().String()[:6]
}
...
type OrderItem struct {
	...
	DeviceID    string `gorm:"size:50" json:"device_id"` // 续费时使用
	SkuID       string `gorm:"size:50" json:"sku_id"`    // 新购时使用
...
type Contract struct {
	...
	SignMode       string `gorm:"size:20" json:"sign_mode"` // online / offline
	ContractStatus string `gorm:"size:30;default:not_generated" json:"contract_status"`
...
type Payment struct {
	...
	Channel       string  `gorm:"size:30;default:placeholder" json:"channel"` // placeholder / wechat / alipay / stripe
...
type Invoice struct {
	...
	InvoiceType   string  `gorm:"size:30;default:normal" json:"invoice_type"` // normal (普票)
