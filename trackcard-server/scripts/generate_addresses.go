...
func main() {
	...
	fmt.Println("=== 开始为停留记录生成地址 ===")

	// 查询所有只有经纬度坐标的记录（地址为经纬度格式）
	rows, err := db.Query(`
		SELECT id, latitude, longitude, address
		FROM device_stop_records
		WHERE (address IS NULL OR address = '' OR address LIKE '%.%')
	...
	for rows.Next() {
		...
		err := rows.Scan(&r.ID, &r.Latitude, &r.Longitude, &addr)
		if err != nil {
	...
	for i, record := range records {
		// 跳过已经是中文地址的记录
		if record.Address != "" && !regexp.MustCompile(`^\d+\.\d+,\s*\d+\.\d+$`).MatchString(record.Address) {
			continue
		}
