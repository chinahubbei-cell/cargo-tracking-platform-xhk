...
func main() {
	// 初始化Nominatim服务（用于逆地理编码）
	services.InitNominatimService("your-email@example.com")
	...
	for i, record := range records {
		fmt.Printf("[%d/%d] 正在获取 %s 的地址...\n", i+1, len(records), record.ID)

		// 调用逆地理编码
		address, err := services.Nominatim.ReverseGeocode(record.Latitude, record.Longitude)
		if err != nil {
			log.Printf("逆地理编码失败 %s: %v", record.ID, err)
			// 使用经纬度作为备用地址
